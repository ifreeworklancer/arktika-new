...
(function () {
    ...
    $('.custom-tabs-nav').on('click', 'li:not(.active)', function () {
        ...
        $(this).closest('section').find('.services-images')
            .each((index, elem) => {
                $(elem).find('.services-images__item').removeClass('active').eq($(this).index()).addClass('active');
            });
    });
    ...
    IMask(
        document.getElementById('user-bottle'),
        {
            mask: /^\d+$/
        });
    ...
    if (!mobilecheck()) {
        ...
        $video.html('<video src="' + $link + '" muted autoplay></video>');
        setTimeout(function () {
            $('.intro-item').addClass('is-visible');
        }, 16000);
    } else {
        $('.intro-item').addClass('is-visible');
    }
    ...
    ScrollReveal().reveal('.services-images', {
        origin: 'right',
        delay: 400,
        distance: '200px',
    });
    ScrollReveal().reveal('.stock-item', {
        origin: 'left',
        delay: 400,
        distance: '200px',
    });
    ScrollReveal().reveal('.stock-card', {
        origin: 'bottom',
        delay: 400,
        distance: '200px',
        interval: 80,
